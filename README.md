...
- 支持逐项处理；批量按钮只选择已确认由任务生成的过程成果，归属不明项仍需逐项决定。
...
2. 打开 `CodexCleaner.exe`。程序默认进入“首页”，存储盘点在后台完成。
3. 点击“清理一个任务”，在全宽任务表中选择任务，再点击“分析所选任务”。首页也会显示适合分析的任务。
4. 选择保留方案，检查“清理、保留、待决定、共享/全局”四组资源。
...
- `crates/app-zsui`：ZSUI 原生 Windows 界面、启动进度和 CLI 入口。
